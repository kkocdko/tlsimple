// from https://github.com/wolfSSL/wolfssl-examples/blob/master/tls/server-tls.c
...
int main() {
  ...
  h(sockfd != SOCKET_INVALID, exit(1), "_");

  WOLFSSL_CTX *ctx = wolfSSL_CTX_new(wolfTLSv1_3_server_method());
  h(ctx != NULL, exit(1), "_");
  h(wolfSSL_CTX_use_certificate_buffer(ctx, CERT_ASN1, sizeof(CERT_ASN1), WOLFSSL_FILETYPE_ASN1) == WOLFSSL_SUCCESS,
    exit(1), "_");
  h(wolfSSL_CTX_use_PrivateKey_buffer(ctx, KEY_ASN1, sizeof(KEY_ASN1), WOLFSSL_FILETYPE_ASN1) == WOLFSSL_SUCCESS,
    exit(1), "_");
  ...
  while (true) {
    ...
    connd = accept(sockfd, (sockaddr *)&clientAddr, &clientAddrSize);
    h(connd != -1, exit(1), "_");
    ...
    if (ret != WOLFSSL_SUCCESS) {
      log(1, "wolfSSL_accept error = %d\n", wolfSSL_get_error(ssl, ret));
      goto exit;
    }

    log(2, "Client connected successfully");

    /* Read the client data into our buff array */
    memset(buff, 0, sizeof(buff));
    h(wolfSSL_read(ssl, buff, sizeof(buff) - 1) != -1, goto exit, "failed to read");
    ...
    h(wolfSSL_write(ssl, buff, len) == (int)len, goto exit, "failed to read");
