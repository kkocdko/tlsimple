...
body=[...(new
Uint8Array(body))].map(v=>"\\x"+v.toString(16).padStart(2,'0')).join("")
console.log(body)
*/
...
int main() {
  ...
  if ((ret = wolfSSL_CTX_use_certificate_buffer(
           ctx, CERT_BUFFER, sizeof(CERT_BUFFER), SSL_FILETYPE_ASN1)) !=
      WOLFSSL_SUCCESS) {
    fprintf(stderr, "ERROR: failed to load CERT_BUFFER.\n");
    goto exit;
  }

  /* Load server key into WOLFSSL_CTX */
  if ((ret = wolfSSL_CTX_use_PrivateKey_buffer(
           ctx, KEY_BUFFER, sizeof(KEY_BUFFER), SSL_FILETYPE_ASN1)) !=
      WOLFSSL_SUCCESS) {
    fprintf(stderr, "ERROR: failed to load KEY_BUFFER.\n");
    goto exit;
  }
