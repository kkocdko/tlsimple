...
int main() {
  ...
  ret = mbedtls_x509_crt_parse(&srvcert, CERT_ASN1, sizeof(CERT_ASN1)); // same as _der?
  h(ret == 0, exit(1), "-0x%x", -ret);

  mbedtls_pk_context pkey;
  mbedtls_pk_init(&pkey);
  ret = mbedtls_pk_parse_key(&pkey, KEY_ASN1, sizeof(KEY_ASN1), NULL, 0, mbedtls_ctr_drbg_random, &ctr_drbg);
  h(ret == 0, exit(1), "-0x%x", -ret);
