...
impl TlsConfig {
    fn new(cert: &[u8], key: &[u8]) -> Self {
        ...
        unsafe {
            ...
            // safety: cert and key were cloned by mbedtls_xxx_parse function
            mbedtls_x509_crt_init(srvcert_p);
            ret = mbedtls_x509_crt_parse(srvcert_p, cert.as_ptr(), cert.len());
            assert_eq!(ret, 0);
...
pub fn run() {
    let tls_config = TlsConfig::new(CERT_DER, KEY_DER);
    // let listener = TcpListener::bind("127.0.0.1:11111").unwrap();

    // let mut entropy = t_init(mbedtls_entropy_init);
    // mbedtls_entropy_init(entropy.as_mut_ptr());
    // let mut entropy = entropy.assume_init();
    // let entropy_p = &mut entropy as *mut _;

    // let mut ctr_drbg = MaybeUninit::<mbedtls_ctr_drbg_context>::uninit();
    // mbedtls_ctr_drbg_init(ctr_drbg.as_mut_ptr());
    // // let mut ctr_drbg = ctr_drbg.assume_init();
    // let pers = "tlsimple";
    // assert!(
    //     mbedtls_ctr_drbg_seed(
    //         ctr_drbg.as_mut_ptr(),
    //         Some(mbedtls_entropy_func),
    //         entropy.as_mut_ptr() as *mut c_void,
    //         pers.as_ptr(),
    //         pers.len(),
    //     ) == 0,
    // );
