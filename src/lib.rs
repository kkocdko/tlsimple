...
impl TlsConfig {
    unsafe fn new_inplace<'a>(
        place: &'a mut MaybeUninit<Self>,
        cert_der: &[u8],
        key_der: &[u8],
    ) -> Pin<&'a mut Self> {
...
impl<'a, S: Read + Write> TlsStream<'a, S> {
    unsafe extern "C" fn bio_send(ctx: *mut c_void, buf: *const u8, len: usize) -> i32 {
        let stream = &mut *(ctx as *mut S);
        match stream.write(slice::from_raw_parts(buf, len)) {
            Ok(n) => n as _,
            Err(e) => {
                dbg!(e);
                -1
    ...
    unsafe extern "C" fn bio_recv(ctx: *mut c_void, buf: *mut u8, len: usize) -> i32 {
        let stream = &mut *(ctx as *mut S);
        match stream.read(slice::from_raw_parts_mut(buf, len)) {
            Ok(n) => n as _,
            Err(e) => {
                dbg!(e);
                -1
    ...
    // unsafe extern "C" fn bio_recv_timeout<S>(ctx: *mut c_void, buf: *mut u8, len: usize) -> i32 {}

    fn new(tls_config: &'a TlsConfig, stream: &'a mut S) -> TlsStream<'a, S> {
        unsafe {
            let ret;
            let mut ssl = MaybeUninit::uninit();
            let ssl_p = ssl.as_mut_ptr();
            mbedtls_ssl_init(ssl_p);
            ret = mbedtls_ssl_setup(ssl_p, &tls_config.conf as _);
            assert_eq!(ret, 0);
            mbedtls_ssl_session_reset(ssl_p);
            mbedtls_ssl_set_bio(
                ssl_p,
                stream as *mut S as _,
                Some(Self::bio_send),
                Some(Self::bio_recv),
                None,
            );
    ...
    fn get_ciphersuite(&self) -> &'static str {
        unsafe {
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        unsafe {
            ...
            match ret {
                ...
                MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY => Err(io::Error::new(
                    io::ErrorKind::Other,
                    "MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY",
                )),
                _ if ret <= 0 => {
                    dbg!(ret);
                    Err(io::Error::new(io::ErrorKind::Other, "other error"))
                }
                _ => Ok(ret as _),
            }
        }
...
pub fn run() {
    unsafe {
        let mut tls_config = MaybeUninit::uninit();
        let mut tls_config = TlsConfig::new_inplace(&mut tls_config, CERT_DER, KEY_DER);
        ...
        loop {
            ...
            write_buf += "Hello world! ciphersuite = ";
