...
pub const SERVER_ADDR: SocketAddr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 11111);

async fn fetch_data(uri: &str) -> hyper::Result<Vec<u8>> {
    let request = Request::get(uri).body(Body::empty()).unwrap();
    let response = tlsimple::fetch(request).await?;
    to_bytes(response.into_body()).await.map(|v| v.to_vec())
}

const _TIMEOUT: Duration = Duration::from_secs(75); // https://nginx.org/en/docs/http/ngx_http_core_module.html#keepalive_timeout
