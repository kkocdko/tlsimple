...
const SERVER_ADDR: SocketAddr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 11111);
const TIMEOUT: Duration = Duration::from_secs(75); // https://nginx.org/en/docs/http/ngx_http_core_module.html#keepalive_timeout
const TO_HTTPS_PAGE: &[u8] = b"HTTP/1.1 200 OK\r\ncontent-type:text/html\r\n\r\n<script>location=location.href.replace(':','s:')</script>\r\n\r\n\0";
const DEMO_PAGE_SMALL: &str = "<!DOCTYPE html><head></head><body><h1>DEMO_PAGE_SMALL</h1></body>";

pub fn run_server_sync() {
    let tls_config = TlsConfig::new_server(CERT_DER.into(), KEY_DER.into(), None);
...
pub fn run_client_sync() {
    let tls_config = TlsConfig::new_client(Some(Vec::new()));
    let mut stream = std::net::TcpStream::connect("archlinux.org:443").unwrap();
    let mut tls_stream = TlsStream::new_sync(tls_config, &mut stream);

    let write_buf = b"GET https://archlinux.org/packages/extra/x86_64/rust/json/ HTTP/1.1\r\nhost: archlinux.org\r\n\r\n";
    tls_stream.write_all(write_buf).unwrap();

    loop {
        let mut buf = [0; 128];
        let len = match tls_stream.read(&mut buf) {
            Ok(0) => break,
            Ok(n) => n,
            Err(e) => {
                let inner = e.into_inner().unwrap();
                dbg!(&inner);
                if format!("{inner}") == "MBEDTLS_ERR_SSL_WANT_READ" {
                    continue;
                }
                if format!("{inner}") == "MBEDTLS_ERR_SSL_RECEIVED_NEW_SESSION_TICKET" {
                    continue;
                }
                break;
            }
...
pub async fn serve_tlsimple(addr: &SocketAddr, svc: axum::Router) {
    // pub async fn serve_tlsimple(addr: &SocketAddr, svc: axum::Router) {
    let tls_config = TlsConfig::new_server(CERT_DER.into(), KEY_DER.into(), Some(alpn::H1));
...
#[cfg(feature = "non-existent")]
pub async fn serve_raw(addr: &SocketAddr, svc: axum::Router) {
    let listener = tokio::net::TcpListener::bind(addr).await.unwrap();
    loop {
...
pub async fn run_server_async_axum() {
    let addr = SocketAddr::from(([0, 0, 0, 0], 9304));
    #[cfg(feature = "non-existent")]
    let app = axum::Router::new()
        .route(
            "/",
            axum::routing::get(([("Connection", "close")], DEMO_PAGE_SMALL)),
        )
        .with_state(());
    #[cfg(feature = "non-existent")]
...
pub async fn run_client_async() {
    ...
    let req = Request::builder()
        ...
        .uri("https://archlinux.org/packages/extra/x86_64/rust/json/")
        .header("Host", "archlinux.org")
        .method("GET")
...
#[tokio::test]
async fn test_self_server_client() {
    // TODO
}

#[tokio::test]
async fn test_server_curl() {
    // TODO
}
